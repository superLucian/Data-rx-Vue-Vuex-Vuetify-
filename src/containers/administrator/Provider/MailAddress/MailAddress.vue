    ...
    <v-container fluid class="px-0 my-sub-scroll-enabled">
      <v-layout row wrap>
        <v-flex xs6 class="px-3">
          <v-text-field label="ADDRESS1" v-model="info.mail_addr1_txt"></v-text-field>
        </v-flex>
        <v-flex xs6 class="px-3">
          <v-text-field label="ADDRESS2" v-model="info.mail_addr2_txt"></v-text-field>
        </v-flex>
        <v-flex xs6 class="px-3">
          <v-text-field label="CITY" v-model="info.mail_city_txt"></v-text-field>
        </v-flex>
        <v-flex xs3 class="px-3">
          <v-select
            v-bind:items="state_items"
            v-model="info.mail_state_cd"
            label="State"
            item-value="text"
          ></v-select>
        </v-flex>
        <v-flex xs3 class="px-3">
          <v-text-field label="Zip Code" v-model="info.mail_zip_cd"></v-text-field>
        </v-flex>
      </v-layout>
    </v-container>
...
<script>
  ...
  export default {
    ...
    mounted() {
      console.log('----------- Administrator / Provider / Edit / MailAddress_tab mounted! -----------')
        ...
        if (this.info.nabp === ''
          ...
          providerServices.updateProvider(this.info, this.$store.state.user, this.$store.state.pcn.pcc).then(function (res) {
            if (res === '0') {
              ...
              window.Vue.$emit('snackbar', 'success', 'Provider MailAddress Updated!')
            } else {
