  ...
  <main>
    <v-card-text class="my-scroll-enabled">
      <v-layout row wrap>
        ...
        <!-- Provider Name tab-->
        <v-flex xs12 v-if="finderMode=='provider_name'">
          <v-layout row wrap>
            <v-flex xs12>
            ...
            <v-flex xs6>
              <v-card-text class="py-0">
                <v-text-field label="Provider Name" v-model="finderKey['provider_name']"></v-text-field>
              </v-card-text>
            </v-flex>
            <v-flex xs6></v-flex>
            <v-flex xs5>
              <v-card-text class="py-0">
                <v-text-field label="City" v-model="finderKey['city']"></v-text-field>
              </v-card-text>
            </v-flex>
            <v-flex xs3>
              <v-card-text class="py-0">
                <v-select label="State" v-bind:items="stateList" v-model="finderKey['selectedState']"></v-select>
              </v-card-text>
            </v-flex>
            <v-flex xs4>
              <v-card-text class="py-0">
                <v-text-field v-model="finderKey['zip']" label="Zip Code"></v-text-field>
              </v-card-text>
            </v-flex>
          </v-layout>
        </v-flex>
        <!-- NABP/NPI tab-->
        <v-flex xs12 v-if="finderMode=='nabp_npi'">
          <v-layout row wrap>
            <v-flex xs6>
              <v-card-text>
            ...
            <v-flex xs6>
              <v-card-text>
                <v-text-field label="Enter the NABP or NPI of the provider"
                              v-model="finderKey['nabp_npi']"></v-text-field>
              </v-card-text>
            </v-flex>
            <v-flex xs12>
              <v-card-text>
        ...
        <!-- Store# tab-->
        <v-flex xs12 v-if="finderMode=='store'">
          <v-layout row wrap>
            <v-flex xs6>
              <v-card-text>
                You can search for a specific provider or a list of providers using the selection option below.
            ...
            <v-flex xs6>
              <v-card-text>
                <v-text-field label="Enter the store number" v-model="finderKey['store']"></v-text-field>
              </v-card-text>
            </v-flex>
            <v-flex xs12>
              <v-card-text>
  ...
  export default {
    ...
    watch: {
      finderMode: function () {
        ...
        this.finderKey['provider_name'] = ''
        this.finderKey['city'] = ''
        this.finderKey['selectedState'] = ''
        this.finderKey['nabp_npi'] = ''
        this.finderKey['store'] = ''
        this.foundWholesalerProviderList = []
